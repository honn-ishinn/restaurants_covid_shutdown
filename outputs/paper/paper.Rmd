...
&nbsp; &nbsp; Our data explores shutdown effects on restaurant businesses in Ontario. We analyzed it using R [@citeR], and packages `tidyverse` [@cite_tidyverse], `stringr`[@cite_stringr],  `here` [@cite_here], `ggpubr`[@cite_ggpubr]. We used R packages `cansim` [@cite_cansim], `httr`[@cite_httr], `xml2`[@cite_xml2] to decide our intervention and sampling method, and packages `bookdown` [@cite_bookdown], `kableExtra`[@cite_kableextra], `ggrepel`[@cite_ggrepel] to format the document. We referenced `Impact Evaluation in Practice`[@cite_impact] and `Sampling Theory and Practice`[@cite_sampletheory] to design our experiment.
