...
#Q1
#```{r}
table(simulated_dataset$q1_city,simulated_dataset$q2_type)%>%
  addmargins()%>%
  kable("html", align = 'clc', caption = 'Restaurant type and quantity information table')%>%
...
data.frame(table(simulated_dataset$q8_survive,simulated_dataset$q1_city))%>%
    mutate(Var1 = fct_relevel(Var1 ,"Prefer not to say","Unknown","Longer than 6 months","3 to 6 months","Less than 3 months")) %>% 
  ggplot(aes(Var1, Freq,fill=Var2)) +
  theme_minimal()+
...
data.frame(table(simulated_dataset$q10_recover,simulated_dataset$q1_city,simulated_dataset$q2_type))%>%
  ...
  ggtitle("How long will it take to recover?")+
  guides(fill=guide_legend(title="City"))+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 90))+
...
t1<-simulated_dataset%>%
  mutate(Average_value= case_when(
    q1_city=="Brantford"~ round(mean(simulated_dataset[simulated_dataset$q1_city=="Brantford",]$q11_rate_support),3),
    q1_city=="Peterborough" ~ round(mean(simulated_dataset[simulated_dataset$q1_city=="Peterborough",]$q11_rate_support),3)
  ))%>%
  select(q1_city,q11_rate_support,Average_value,3)



table(t1$q1_city,t1$q11_rate_support)%>%
  addmargins()%>%
  kable("html", align = 'clc', caption = 'Number and level of ratings from two cities')%>%
  kable_styling(full_width = F)
