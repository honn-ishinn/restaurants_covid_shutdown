...
tests <- simulated_survey1_dataset %>% 
 ...
 tests$q1_city <- NULL
#  tests$Responses[c(1:5,9,8,6,7,10,11,13,14,15,12,16,21,17,18,19,20,22),] # Manually change the order of the factor levels to have a better visualization 
 tests <- tests[c(1:5,9,8,6,7,10,11,13,14,15,12,16,21,17,18,19,20,22),] %>% 
    mutate(group = c("Total Samples","Restaurant Category","","Restaurant Nature","","Number of Employees","","","","","Average Monthly Sales","","","","","","Restaurant Capacity","","","","",""))
...
tests %>% 
  ...
  kable( tests[,c(4,1,2,3)],caption = "TEST CAPTION",
         booktabs = TRUE,
         valign='t')
    ) %>% 
  kable_styling()
